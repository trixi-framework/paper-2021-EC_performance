...
function gather_results(filename_start)
  ...
  for (root, dirs, files) in walkdir(@__DIR__)
    for filename in files
      if startswith(filename, filename_start)
...
gather_results("pids_2D_flux_shima_etal_")
gather_results("pids_3D_flux_shima_etal_")

gather_results("pids_2D_flux_ranocha_")
gather_results("pids_3D_flux_ranocha_")
