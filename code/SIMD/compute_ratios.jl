...
## compute relative performance of SIMD optimizations of the volume terms
...
function compute_relative_performance(file_new, file_baseline)
  ...
  open(file_new[1:end-4] * "_relative.dat", "w") do io
    println(io, header)
    writedlm(io, data)
  end
end
...
compute_relative_performance(
  joinpath(@__DIR__, "pids_$(setup)_turbo.dat"),
